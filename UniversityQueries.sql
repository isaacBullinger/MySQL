...
SELECT lname, fname, FLOOR(DATEDIFF('2017-1-5', birthdate)/365) AS 'Years', DATEDIFF('2017-1-5', birthdate)%365 AS 'Days', CONCAT(FLOOR(DATEDIFF('2017-1-5', birthdate)/365),' - Yrs, ',DATEDIFF('2017-1-5', birthdate)%365,' - Days') AS 'Years and Days'
FROM student
ORDER BY birthdate;
...
SELECT department_code, course_num, course_title
FROM student s
	JOIN enrollment e
    ON s.student_id = e.student_id
    JOIN section se
...
SELECT term, year, COUNT(term) AS 'Enrollment'
FROM student s
	JOIN enrollment e
    ON s.student_id = e.student_id
    JOIN section se
...
SELECT college, COUNT(course_id) AS 'Courses'
FROM course c
    JOIN department d
    ON c.department_id = d.department_id
    JOIN college co
...
SELECT s.lname, s.fname, SUM(credits) AS 'Credits'
FROM student s
	JOIN enrollment e
    ON s.student_id = e.student_id
    JOIN section se
